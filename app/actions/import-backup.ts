...
export async function importLocalBackup(_prev: ImportState, formData: FormData): Promise<ImportState> {
  ...
  if (!isRecord(raw) || raw.format !== 'networth-backup' || (raw.version !== 1 && raw.version !== 2) || !isRecord(raw.data)) return { error: 'Choose a NetWorth local backup file.' }
  const { items, snapshots, events, goal, goals } = raw.data
  ...
  const validGoal = (value: unknown) => isRecord(value) && typeof value.id === 'string' && typeof value.name === 'string' && value.name.length > 0 && value.name.length <= 80 && isSafeMoney(value.target_cents) && value.target_cents > 0 && isSafeMoney(value.allocated_cents) && value.allocated_cents >= 0 && value.allocated_cents <= value.target_cents && (value.target_date === null || validDate(value.target_date)) && (value.note === null || typeof value.note === 'string')
  if (raw.version === 2 && (!Array.isArray(goals) || !goals.every(validGoal))) return { error: 'The backup contains invalid goals.' }
  if (raw.version === 1 && goal !== null && (!isRecord(goal) || !isSafeMoney(goal.target_cents) || (goal.target_date !== null && typeof goal.target_date !== 'string') || (goal.note !== null && typeof goal.note !== 'string'))) return { error: 'The backup contains an invalid goal.' }
